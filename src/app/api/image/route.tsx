...
import ImagePost, { normalizeSections } from "./ImagePost";

function loadFont(weight: number): Buffer | null {
  const p = join(process.cwd(), `node_modules/@fontsource/bricolage-grotesque/files/bricolage-grotesque-latin-${weight}-normal.woff`);
...
export async function POST(req: NextRequest) {
  ...
  const { data, theme = "dark" }: { data: ImageTemplateData; theme: ImageTheme } = await req.json();

  type Weight = 100 | 200 | 300 | 400 | 500 | 600 | 700 | 800 | 900;
  const fonts = ([400, 500, 600, 700, 800] as const)
    .map((w) => { const buf = loadFont(w); return buf ? { name: "Bricolage Grotesque", data: buf, weight: w as Weight, style: "normal" as const } : null; })
    .filter(Boolean) as { name: string; data: Buffer; weight: Weight; style: "normal" }[];

  if (!fonts.length) return NextResponse.json({ error: "Font files not found" }, { status: 500 });

  const norm = normalizeSections(data);
  const toolNamesToLoad: string[] = [];
  for (const s of norm.sections) {
    if (s.type === "stats") toolNamesToLoad.push(...(s.toolNames ?? []));
    if (s.type === "tools") toolNamesToLoad.push(...(s.toolList ?? []).map((t) => t.name));
  }
...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ authorPhoto, companyLogo, tools, toolNames: tools.map((t) => t.name) });
}
